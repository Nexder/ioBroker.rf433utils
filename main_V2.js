		...
		try
		{
			...
			else
			{
				for (let k = 0; k < this.config.devices.length; k++) 
				{
					...
					{
						this.log.silly(`Incoming Turn On ${device.name}`);
						this.AddOrUpdateObject(device, true);
					}
					else if (device.codeOff == code)
					{
						this.log.silly(`Incoming Turn Off ${device.name}`);
			...
			else
			{
				for (let k = 0; k < this.config.devices.length; k++) 
				{
					...
					if (device.name == name)
					{
						if (state)
						{
							this.log.debug(`Outgoing Turn On ${device.name} - ${device.codeOn}`);
							await exec(`/home/pi/433Utils/RPi_utils/codesend ${device.codeOn}`);
						}
						else if (!state)
						{	
							this.log.debug(`Outgoing Turn OFF ${device.name} - ${device.codeOff}`);
							await exec(`/home/pi/433Utils/RPi_utils/codesend ${device.codeOff}`);
						}
					}
				}
			}
